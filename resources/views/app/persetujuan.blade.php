...
<div class="grid grid-cols-12 gap-6">
            ...
            <div class="grid grid-cols-12 gap-6">
                ...
                <div class="col-span-12 mt-6">
                    <div class="intro-y block sm:flex items-center h-10">
                        <h2 class="text-lg font-medium truncate mr-5">Data Surat persetujuan</h2>
                        <a href="javascript:;" data-toggle="modal" data-target="#tambah-data-persetujuan"
                        class="btn btn-primary shadow-md mr-2">Tambah Data Surat persetujuan</a>
                                    ...
                                    <tr class="intro-x">
                                        ...
                                        <td class="table-report__action w-56">
                                            <div class="flex justify-center items-center">
                                                <a class="flex items-center mr-3 persetujuanedit" href="javascript:void(0)" data-toggle="modal" 
                                                id="{{ $item->id }}" data-target="#persetujuan-edit" data-id="{{ $item->id }}">
                                                    <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit 
                                                </a>
                                                <a class="flex items-center text-theme-6" href="/persetujuandelete/{{$item->id}}" onclick="return confirm('Apakah Anda Yakin Menghapus Data?');">
                                                    <i data-feather="trash-2" class="w-4 h-4 mr-1"></i> Delete
                                                </a>
                                                <a class="flex items-center text-theme-11" href="{{ route('persetujuancetak', $item->id) }}" >
                    ...
                    <div class="pagination">
                        @if ($persetujuan != null)
                            {{ $persetujuan->appends($data_request)->links() }}
                        @endif
                        </div>
                    </div>
                </div>
        ...
        <div id="tambah-data-persetujuan" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <!-- BEGIN: Modal Header -->
                    <div class="modal-header">
                        <h2 class="font-medium text-base mr-auto">Tambah Data Surat persetujuan</h2>
                    ...
                    <!-- END: Modal Header -->
                    <!-- BEGIN: Modal Body -->
                    <form method="POST" action="{{ route('persetujuanadd') }}">
                        @csrf
                    <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                        <div class="col-span-12 sm:col-span-12">
                            <label for="modal-form-10" class="form-label">persetujuan Nikah</label>
        ...
        <div id="persetujuan-edit" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <!-- BEGIN: Modal Header -->
                    <div class="modal-header">
                    ...
                    <!-- END: Modal Header -->
                    <!-- BEGIN: Modal Body -->
                    <form method="POST" action="{{ route('persetujuanupdate') }}">
                    <input type="hidden" name="id" id="pasangan-update-id">
                        @csrf
                    <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                        <div class="col-span-12 sm:col-span-12">
                            <label for="persetujuan-edit-10" class="form-label">persetujuan Nikah</label>
                            <select id="persetujuan-edit-10" class="form-select" name="pasangan_id">
                                @foreach ($pasangan as $item)
                                <option value="{{ $item->id }}">{{ $item->nama_pasangan . ' dan ' . $item->nama }}</option>
                                @endforeach
                            </select>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $('.persetujuanedit').on('click',function() {
                    var id = $(this).attr('data-id');

                    $.ajax({
                        url : "{{route('persetujuanedit')}}?id="+id,
                        type: "GET",
                        dataType: "JSON",
                        success: function(data)
                        {
                            ...
                            $('#persetujuan-edit-10 option[value="' + data.pasangan_id +'"]').prop("selected", true);
                        }
                    });
                });
            });
