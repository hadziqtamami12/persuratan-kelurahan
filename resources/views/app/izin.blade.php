...
<div class="grid grid-cols-12 gap-6">
            ...
            <div class="grid grid-cols-12 gap-6">
                ...
                <div class="col-span-12 mt-6">
                    <div class="intro-y block sm:flex items-center h-10">
                        <h2 class="text-lg font-medium truncate mr-5">Data Surat izin</h2>
                        <a href="javascript:;" data-toggle="modal" data-target="#tambah-data-izin"
                        class="btn btn-primary shadow-md mr-2">Tambah Data Surat izin</a>
                                    ...
                                    <tr class="intro-x">
                                        ...
                                        <td class="table-report__action w-56">
                                            <div class="flex justify-center items-center">
                                                <a class="flex items-center mr-3 izinedit" href="javascript:void(0)" data-toggle="modal" 
                                                id="{{ $item->id }}" data-target="#izin-edit" data-id="{{ $item->id }}">
                                                    <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit 
                                                </a>
                                                <a class="flex items-center text-theme-6" href="/izindelete/{{$item->id}}" onclick="return confirm('Apakah Anda Yakin Menghapus Data?');">
                                                    <i data-feather="trash-2" class="w-4 h-4 mr-1"></i> Delete
                                                </a>
                                                <a class="flex items-center text-theme-11" href="{{ route('izincetak', $item->id) }}" >
                    ...
                    <div class="pagination">
                        @if ($izin != null)
                            {{ $izin->appends($data_request)->links() }}
                        @endif
                        </div>
                    </div>
                </div>
        ...
        <div id="tambah-data-izin" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <!-- BEGIN: Modal Header -->
                    <div class="modal-header">
                        <h2 class="font-medium text-base mr-auto">Tambah Data Surat izin</h2>
                    ...
                    <!-- END: Modal Header -->
                    <!-- BEGIN: Modal Body -->
                    <form method="POST" action="{{ route('izinadd') }}">
                        @csrf
                    <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                        <div class="col-span-12 sm:col-span-12">
                            <label for="modal-form-10" class="form-label">izin Nikah</label>
        ...
        <div id="izin-edit" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <!-- BEGIN: Modal Header -->
                    <div class="modal-header">
                    ...
                    <!-- END: Modal Header -->
                    <!-- BEGIN: Modal Body -->
                    <form method="POST" action="{{ route('izinupdate') }}">
                    <input type="hidden" name="id" id="pasangan-update-id">
                        @csrf
                    <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                        <div class="col-span-12 sm:col-span-12">
                            <label for="izin-edit-10" class="form-label">izin Nikah</label>
                            <select id="izin-edit-10" class="form-select" name="pasangan_id">
                                @foreach ($pasangan as $item)
                                <option value="{{ $item->id }}">{{ $item->nama_pasangan . ' dan ' . $item->nama }}</option>
                                @endforeach
                            </select>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $('.izinedit').on('click',function() {
                    var id = $(this).attr('data-id');

                    $.ajax({
                        url : "{{route('izinedit')}}?id="+id,
                        type: "GET",
                        dataType: "JSON",
                        success: function(data)
                        {
                            ...
                            $('#izin-edit-10 option[value="' + data.pasangan_id +'"]').prop("selected", true);
                        }
                    });
                });
            });
